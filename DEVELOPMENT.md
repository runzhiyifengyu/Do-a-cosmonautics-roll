...
当前状态：阶段 1 实现完成；首跑 gradlew 权限失败已修复（chmod +x + LF）；二跑编译失败已修复（CustomPacketPayload 改为 Minecraft 原生包 `net.minecraft.network.protocol.common.custom`），等待用户重新 push 验证构建、确认并 commit 后进入阶段 2
